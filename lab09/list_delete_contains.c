// COMP1511 Week 9 LAB: list_delete_contains.c
//
// This program was written by Arya Bodhi Gunawan (z5240037)
// on 1 AUGUST 2021
//
// This program deletes first node in the 
// linked list that whose data field equals value
...
struct node *delete_contains(int value, struct node *head) {
    ...
    if (current->next != NULL && current->next->data == value) {
        struct node *val = current->next;
        current->next = val->next;
        free(val);
    }
    return head;
